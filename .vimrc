...
noremap { :tp<CR>
noremap } :tn<CR>
noremap g<C-t> :!np3tags.pl -q . ..\common<CR>
...
noremap g<C-h> :!txt2tags --encoding utf-8 -t html --style ./style.css -i "%:p"<CR>
"noremap g<C-h> :!txt2tags --encoding iso8859-1 -t html --toc --style ./style.css -i "%:p"<CR>
...
fun! s:LongLineHLToggle()
 if !exists('w:longlinehl')
  highlight OverLength  ctermbg=red ctermfg=none guibg=gray20
  highlight OverLength0 ctermbg=red ctermfg=none guibg=gray25
  highlight OverLength100 ctermbg=red ctermfg=none guibg=gray30
  let w:longlinehl  = matchadd('OverLength',  '.\%>81v', -1)
  let w:longlinehl0 = matchadd('OverLength0', '.\%82v',  -1)
  let w:longlinehl100 = matchadd('OverLength100', '.\%>101v',  -1)
  echo "Long lines highlighted"
 else
  call matchdelete(w:longlinehl)
  unl w:longlinehl
  call matchdelete(w:longlinehl0)
  unl w:longlinehl0
  call matchdelete(w:longlinehl100)
  unl w:longlinehl100
...
" Highlight code snippets potentialy with pattern errors
map <F11> /[^ =><!\&\|*/+-]=\\|=[^ =]\\|[^ -][><]\\| [><][^ =]\\|\(if(\)\\|\(while(\)\\|\(switch(\)\\|\(for(\)\\|\(do{\)\\|while.*(.*)\n[ \t]*{\\|switch.*(.*)\n[ \t]*{\\|if.*(.*)\n[ \t]*{\\|for.*(.*)\n[ \t]*{\\|else.*\n[ \t]*{\\|[,;][^ \t]\\|[ \t]\n<Esc><Return>:<Esc>
imap <F11> <Esc> /[^ =><!\&\|*/+-]=\\|=[^ =]\\|[^ -][><]\\| [><][^ =]\\|\(if(\)\\|\(while(\)\\|\(switch(\)\\|\(for(\)\\|\(do{\)\\|while.*(.*)\n[ \t]*{\\|switch.*(.*)\n[ \t]*{\\|if.*(.*)\n[ \t]*{\\|for.*(.*)\n[ \t]*{\\|else.*\n[ \t]*{\\|[,;][^ \t]\\|[ \t]\n<Esc><Return>:<Esc>
